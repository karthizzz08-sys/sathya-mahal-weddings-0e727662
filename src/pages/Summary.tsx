...
export default function Summary() {
  ...
  // Add catering subtotal if menus are selected
  const cateringSubtotal = state.morningMenuTotal + state.lunchMenuTotal + state.eveningMenuTotal;
  if (cateringSubtotal > 0) {
    lines.push({ label: "Catering Subtotal", value: `Morning + Lunch + Evening Menus = ₹${cateringSubtotal.toLocaleString()}` });
  }
  
  if (state.morningMenuTotal > 0) lines.push({ label: "  → Morning Menu", value: `${state.tiffen?.name || "Menu"} × ${state.guests} guests = ₹${state.morningMenuTotal.toLocaleString()}` });
  if (state.lunchMenuTotal > 0) lines.push({ label: "  → Lunch Menu", value: `${state.lunch?.name || "Menu"} × ${state.guests} guests = ₹${state.lunchMenuTotal.toLocaleString()}` });
  if (state.eveningMenuTotal > 0) lines.push({ label: "  → Evening Menu", value: `${state.evening?.name || "Menu"} × ${state.guests} guests = ₹${state.eveningMenuTotal.toLocaleString()}` });
